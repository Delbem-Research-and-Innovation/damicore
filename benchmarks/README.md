...
Measurements always go to stdout; `--output` also writes them as JSON to a path. The weekly
workflow uses it and uploads the files as a run artifact, because the regression rule below
compares against earlier runs and stdout of a finished job cannot be read back. A budget
breach is reported after the measurements are written, so the failing numbers survive too.
